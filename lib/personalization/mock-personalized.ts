 ...
 * Mock LLM 출력 3종.
 *
 * 카피 톤: 합쇼체, 호칭 "의뢰인님", 답변 큰따옴표 직접 인용.
 * 모든 문장은 [LLM_GENERATED] 자리 — 추후 실제 LLM 응답으로 교체 예정.
 *
 * 향후 LLM 연동 시 이 파일을 `fallback-personalized.ts`로 rename 하고
 * LLM 실패 폴백으로 재활용.
 */
...
export const MOCK_PERSONALIZED_A: PersonalizedContent = {
  chapter1Traits: {
    // [LLM_GENERATED] Trait 01 — relationshipPriority, contactStyle 기반
    trait01Intro: '의뢰인님께서는 연애에서 "매일 연락하는 거"를 가장 중요하게 꼽으셨습니다.',

    // [LLM_GENERATED] Trait 02 — attractionFactor 기반
    trait02Intro: '"성격이 좋은 사람"에게 끌린다고 답하셨듯이,',

    // [LLM_GENERATED] Trait 03 — 관련 답변 약함 → 폴백에 가까움
    trait03Intro: '의뢰인님의 선택지에서 섬세함에 대한 기대가 읽힙니다.',

    // [LLM_GENERATED] Trait 04 — dealBreaker 기반
    trait04Intro: '"흡연하는 사람"을 명확히 거절하신 점은,',
  },
  readingCard: {
    // [LLM_GENERATED] 문단 1 — 이상형 답변 2개 통합
    paragraph1Opening:
      '의뢰인님께서 "매일 연락하는 거"를 연애의 우선순위로 꼽으신 점, 그리고 "같은 종교"를 선호하신 점에서,',

    // [LLM_GENERATED] 문단 2 — 자유응답·본인정보 없음 → 폴백
...
export const MOCK_PERSONALIZED_B: PersonalizedContent = {
  chapter1Traits: {
    ...
    // [LLM_GENERATED]
    trait02Intro: '"유능한 능력을 가진 사람"에 끌리며, 자기 관리에 대한 기준을 명확히 갖고 계신 점에서,',

    // [LLM_GENERATED]
    trait03Intro: '자유 응답에서 "손톱, 피부, 머리카락 등 기본적인 자기 관리"를 언급하신 것처럼,',

    // [LLM_GENERATED]
    trait04Intro: '"흡연하는 사람"을 명확히 거절하셨고, 자기 관리에 대한 기대치가 뚜렷한 점을 보면,',
  },
  readingCard: {
    // [LLM_GENERATED]
    paragraph1Opening:
...
export const MOCK_PERSONALIZED_C: PersonalizedContent = {
  chapter1Traits: {
    // [LLM_GENERATED]
    trait01Intro: '"같이 있을 때 편한 거"를 가장 중요하게 꼽으셨고, 연락 빈도에는 유연한 태도를 보이셨습니다.',
  ...
  readingCard: {
    ...
    // [LLM_GENERATED] 자유응답 없음, 본인 정보 활용
    paragraph2Opening:
